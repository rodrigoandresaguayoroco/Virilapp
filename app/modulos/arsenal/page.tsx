...
const tactics: Tactic[] = [
  {
    ...
    protocol: [
      'Inhala 4 s por nariz (diafragma)',
      'Pausa 2 s',
      'Exhala 6 s por boca',
      'Sincroniza respiración y movimiento'
    ],
  ...
  },
  {
    ...
    protocol: [
      'Identifica el PC al cortar el flujo de orina',
      'Realiza el movimiento opuesto (empuje suave)',
      'Entrena contracción 2 s y relajación 10 s, priorizando la relajación'
    ],
  ...
  },
  {
    ...
    protocol: [
      'Usa un medidor interno 1–10',
      'En 7–8, detén la estimulación',
      'Respira y baja a 3–4',
      'Reanuda cuando el control regrese'
    ],
  ...
  },
  {
    ...
    protocol: [
      'Observación activa de respiración, sonidos y expresiones',
      'Escucha y mira conscientemente',
      'Haz del placer de tu pareja tu objetivo operativo'
    ],
  ...
  },
  {
    ...
    protocol: [
      'Evita ritmo constante por tiempo prolongado',
      'Alterna rápido/superficial con lento/profundo',
      'Sincroniza fases lentas con respiración'
    ],
  ...
  },
  {
    ...
    protocol: [
      ...
      'Cambia posición para modificar ángulo y profundidad',
      'Activa zonas erógenas con manos y boca'
    ],
  ...
  },
  {
    ...
    protocol: [
      'Habla antes en un contexto relajado',
      'Acordad una señal discreta',
      'Agradece y refuerza tras la intimidad'
    ],
...
export default function ArsenalPage() {
  ...
  useEffect(() => {
    ...
    if (saved) {
      setCompletedTactics(JSON.parse(saved))
    }
  }, [])
  ...
  const markAsCompleted = (tacticId: string) => {
    ...
    const progress = {
      arsenal: Math.round((newCompleted.length / tactics.length) * 100),
    ...
    localStorage.setItem('viril-progress', JSON.stringify(progress))
  }
      ...
      <header className="w-full p-6 border-b border-white/10">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          ...
          <div className="flex items-center space-x-4">
            <Button
              variant="outline"
              onClick={() => setShowBreathingTimer(!showBreathingTimer)}
              className="border-viril-400 text-viril-400 hover:bg-viril-400 hover:text-viril-950"
            >
              <Timer className="w-4 h-4 mr-2" />
              {showBreathingTimer ? 'Ocultar' : 'Mostrar'} Timer
            </Button>
            <Button
              variant="ghost"
              onClick={() => {
                const link = document.createElement('a')
                link.href = '/upload/Guia_2_Arsenal_del_Amante_v2.pdf'
                link.download = 'Guia_2_Arsenal_del_Amante_v2.pdf'
                link.click()
              }}
              className="text-gray-400 hover:text-white"
            >
          ...
          <Card className="bg-white/5 backdrop-blur-sm border-white/10">
            <CardHeader>
              <CardTitle className="text-3xl text-gradient mb-4">
                De la Ansiedad a la Maestría
              </CardTitle>
              <CardDescription className="text-lg text-gray-300">
                Bienvenido a tu arsenal táctico. El rendimiento sexual no es suerte: es una habilidad entrenable. 
                Aquí encontrarás 7 tácticas basadas en fisiología y psicología para aumentar resistencia e 
                intensificar el placer, con foco en control consciente y presencia.
              </CardDescription>
            </CardHeader>
          </Card>
        </div>
          ...
          {tactics.map((tactic, index) => (
            <Card 
              ...
              className={`bg-white/5 backdrop-blur-sm border-white/10 transition-all duration-300 ${
                completedTactics.includes(tactic.id) ? 'border-green-400/50 bg-green-400/5' : ''
              }`}
            >
              <CardHeader>
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center space-x-3">
                    <div className="p-3 rounded-lg bg-gradient-to-r from-viril-400 to-viril-700 text-white">
                      {tactic.icon}
                    </div>
                    <div>
              ...
              <CardContent>
                <Accordion type="single" collapsible className="space-y-4">
                  <AccordionItem value="protocol" className="border-white/10">
                    ...
                    <AccordionContent>
                      <ol className="space-y-2">
                        {tactic.protocol.map((step, stepIndex) => (
                  ...
                  <AccordionItem value="science" className="border-white/10">
                    ...
                    <AccordionContent>
                      <p className="text-gray-300 bg-blue-500/10 p-4 rounded-lg border-l-4 border-blue-400">
                        {tactic.science}
                      </p>
                    </AccordionContent>
                  </AccordionItem>
                ...
                {!completedTactics.includes(tactic.id) && (
                  <div className="mt-6 pt-6 border-t border-white/10">
                    <Button
                      onClick={() => markAsCompleted(tactic.id)}
          ...
          <Card className="bg-yellow-500/10 border-yellow-400/30">
            ...
            <CardContent>
              <p className="text-gray-300">
                Material educativo; no sustituye valoración médica. Si hay dolor persistente, 
                ansiedad marcada o disfunciones que afecten tu bienestar, consulta a un profesional de salud sexual.
              </p>
            </CardContent>
          </Card>
        </div>
